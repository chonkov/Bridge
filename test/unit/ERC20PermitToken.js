...
describe("ERC20PermitToken", function () {
  async function ERC20PermitToken() {
    const [signer, ...other] = await ethers.getSigners();

    const myToken = await ethers.deployContract("ERC20PermitToken", [
      "MyToken",
      "MTKN",
    ]);
  ...
  describe("Deployment", function () {
    it("Should set the right metadata", async function () {
      const { myToken } = await loadFixture(ERC20PermitToken);
      const name = "MyToken";
      const symbol = "MTKN";
    ...
    it("Should set the right owner", async function () {
      const { myToken, signer } = await loadFixture(ERC20PermitToken);
  ...
  describe("Minting", function () {
    ...
    it("Should throw an error", async function () {
      const { myToken, signer, other } = await loadFixture(ERC20PermitToken);
      const value = ethers.parseEther("1");
